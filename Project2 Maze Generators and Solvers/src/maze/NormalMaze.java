		...
		for (int i = 0; i < sizeR; i++){
			for (int j = 0; j < sizeC; j++) {
				...
				for (int k = 3; k < NUM_DIR; k++) {
					if (k == 4) continue;
					// check this cell's down and left is empty or not, if it has a cell
					if (isIn(i + deltaR[k], j + deltaC[k])) {
						// when k = 3 or 5, it is left cell or down cell (a neighbor for the recent cell)
						Cell neigh = map[i + deltaR[k]][j + deltaC[k]];
						// recent cell's left or down wall is signed, its neighbor's right wall or top wall 
						cell.wall[k] = neigh.wall[oppoDir[k]];	// WEST and SOUTH walls might have already been created by previous cells
						// connect its neighbor wall with each other
	...
	public boolean isPerfect() {
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < NUM_DIR; i++) {
				...
				// ignore if nextCell is invald or the wall between currCell and nextCell exists 
				// exist means this is not a connect cell, don need to consider
				...
				if (visited[nextCell.r][nextCell.c])				
					...
					// if nextCell is valid and there's no wall, add nextCell to the queue
					queue.add(nextCell);							
			}
		...
		// checking if all cells have been visited by the BFS, if no, return false (not perfect)
		// this cell is been keeping filled
		for (int i = 0; i < sizeR; i++)		
			for (int j = 0; j < sizeC; j++)
				if (!visited[i][j])
					return false;
